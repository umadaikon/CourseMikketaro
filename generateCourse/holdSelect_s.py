...
if __name__ == '__main__':
	...
	os.makedirs(os.path.join("..", "course", user, "deadpoint"), exist_ok=True)
	os.makedirs(os.path.join("..", "course", user, "crossmove"), exist_ok=True)
	os.makedirs(os.path.join("..", "course", user, "diagonal"), exist_ok=True)
	os.makedirs(os.path.join("..", "course", user, "dropknee"), exist_ok=True)
	os.makedirs(os.path.join("..", "course", user, "dyno"), exist_ok=True)
	...
	# for hand in range(2):#hand==0 左手 hand==1 右手
		...
		while(not stackHold == []):
			...
			checkedHold = get_unique_list(checkedHold)
		return course[hand]
	course = joblib.Parallel(n_jobs=-1, verbose=10)([joblib.delayed(GenerateCourse)(h) for h in range(2)])
	#-----------コース生成終了
	# print()
	...
	if not course[0] == [] and not course[1] == []:
		...
		if not len(route) == 0:
			...
			if len(route) > 10:
				outRouteNo = random.sample(range(len(route)), 10)
				outRouteNo.sort()
			else:
				outRouteNo = range(len(route))
			# #値確認
			...
			def DrawCourse(i, no):
				...
				cv2.imwrite(os.path.join("..", "course", user, practiceMove, practiceMove+str(i)+".png"), courseImg)
			...
			joblib.Parallel(n_jobs=-1, verbose=3, backend="threading")([joblib.delayed(DrawCourse)(i, no) for i, no in enumerate(outRouteNo)])
