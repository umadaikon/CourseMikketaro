...
for row in range(5):
	...
	for i, row in enumerate(cur.fetchall()):
		# print(row)
		move[i][0] = i+1     #id
		move[i][1] = row[1] * pHeight/standardHeight # * (imgPixel/basePixel) #p_distance
		move[i][3] = row[3] * pHeight/standardHeight # * (imgPixel/basePixel) #m_distance
		# if count == 5:
		# 	move[i][3] = move[i][3]
...
# 	nonMoveHoldList = [[],[]]
# 	nonMoveHoldList[0] = Parallel(n_jobs=-1, verbose=3)([delayed(getNonMoveRangeHold)(hold, nonMoveRawRange, holdPoint, 0) for hold in holdPoint])
# 	nonMoveHoldList[1] = Parallel(n_jobs=-1, verbose=3)([delayed(getNonMoveRangeHold)(hold, nonMoveRawRange, holdPoint, 1) for hold in holdPoint])
...
if os.path.isfile(moveHoldListName):
	...
	moveHoldList = [[],[]]
	moveHoldList[0] = Parallel(n_jobs=-1, verbose=0)([delayed(getMoveRangeHold)(hold, moveData, holdPoint, 0) for hold in holdPoint])
	moveHoldList[1] = Parallel(n_jobs=-1, verbose=0)([delayed(getMoveRangeHold)(hold, moveData, holdPoint, 1) for hold in holdPoint])
